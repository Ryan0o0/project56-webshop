...
def displayCart(sessionkey):
    ...
    for e in ShoppingCart.objects.all().filter(session_key=sessionkey):
        ...
        html += "<div class='textplace'><button class='remove'><i class='fa fa-trash' aria-hidden='true'></i><p>verwijderen</p></button><button class='movetowishlist'><i class='fa fa-heart' aria-hidden='true'></i><p>verplaats naar verlanglijstje</p></button><p class='title'>" + e.prodNum.prodName + "</p>"
